...
from encryption.encoding import EncodedNumber
...
def generate_paillier_keypair(n_length=DEFAULT_KEYSIZE, sigma_length=DEFAULT_SIGMA):
    ...
    Args:
      ...
      count_partialprivatekey: the count of partially private key

    Returns:
      triple: The generated :class:`PaillierPublicKey`,
      :class:`PaillierPrivateKey`, and :class `PartialPrivateKey`
    """
    p = q = n = None
    n_len = 0
    ...
    #partial_private_keys = PartialPaillierPrivateKey(public_key, private_key, sigma_length)
    partial_private_keys = PartialPaillierPrivateKey(public_key, private_key, (n_length + n_length) - 4)

    return public_key, private_key, partial_private_keys

class PartialPaillierPrivateKey(object):
    """Return two partially private keys.

        Args:
          public_key: pubulic key.
          private_key: private key
          sigma_len: the length of sk1 in bits
    ...
    def __init__(self, public_key, private_key, sigma_len):
        ...
        # lmd = lcm(p-1, q-1) = (p-1)*(q-1)/gcd(p-1, q-1)
        lmd = (private_key.p - 1) * (private_key.q - 1) // extended_euclidean_algorithm(
            private_key.p - 1, private_key.q - 1)[0]
        # lmd_invert = lmd^-1 mod n
        lmd_invert = invert(lmd, self.n)
        self.sk2 = lmd * lmd_invert - self.sk1

class ThresholdPaillier(object):
    """Contains a partially private key and associated threshold decryption methods.

    Args:
      public_key: the public key.
      partial_private_key: the partially private key.
    ...
    Attributes:
      ...
      partial_private_key: the partially private key.
    """
    def __init__(self, public_key, partial_private_key):
        self.public_key = public_key
        self.n = public_key.n
        self.nsquare = public_key.nsquare
        self.partial_private_key = partial_private_key

    def partial_raw_decrypt(self, ciphertext):
        """Partial decryption of a positive integer plaintext < :attr:`nsquare`.

            Args:
                ciphertext (int): a positive integer < :attr:`nsquare`

            Returns:
                int: Partially decrypted ciphertext.
        """
        return powmod(ciphertext, self.partial_private_key, self.nsquare)

    def partial_decrypt(self, encrypted_number):
        """Partial decryption of an EncryptedNumber.

            Args:
                encrypted_number (EncryptedNumber): an instance of EncryptedNumber, consits of :attrs
                public_key (int), ciphertext (int), expoent (int), and __is_obfucated

            Returns:
                PartialDecryptedNumber: a class of partially decrypted ciphertext, consits of :attrs
                public_key (int), ciphertext (int), expoent (int)

            Raises:
                TypeError: if enccrypted_number is not EncryptedNumber.
        """
        if not isinstance(encrypted_number, EncryptedNumber):
            ...
            partial_ciphertext = self.partial_raw_decrypt(encrypted_number.ciphertext(be_secure=False))
            return PartialDecryptedNumber(encrypted_number.public_key, partial_ciphertext, encrypted_number.exponent)

    def final_raw_decrypt(self, partial_decrypted_ciphertext1, partial_decrypted_ciphertext2):
        """Finally decryption of two ciphertexts (int).

            Args:
                partial_decrypted_ciphertext1 (int): the first partially decrypted ciphertext by ThresholdPaillier
                                                     partial_decrypted_ciphertext1 = ciphertext ** sk1 % n * n
                partial_decrypted_ciphertext2 (int): the second partially decrypted ciphertext by ThresholdPaillier
                                                     partial_decrypted_ciphertext2 = ciphertext ** sk2 % n * n

            Returns:
                plaintext (int): the plaintext of a ciphertexted
        """
        plaintext = self.l_function(partial_decrypted_ciphertext1 * partial_decrypted_ciphertext2 % self.nsquare, self.n)
        if plaintext > self.n // 2:
            plaintext = plaintext - self.n
        return plaintext

    def final_decrypt(self, partial_decrypted_number1, partial_decrypted_number2):
        """Finally decryption of two ciphertexts (PartialDecryptedNumber).

            Args:
                partial_decrypted_number1 (PartialDecryptedNumber): the first partially decrypted ciphertext by ThresholdPaillier
                partial_decrypted_number2 (PartialDecryptedNumber): the second partially decrypted ciphertext by ThresholdPaillier

            Returns:
                plaintext: the plaintext of a ciphertexted
        """
        if isinstance(partial_decrypted_number1, PartialDecryptedNumber) and isinstance(partial_decrypted_number2, PartialDecryptedNumber):
            if partial_decrypted_number1.exponent != partial_decrypted_number2.exponent:
                raise Exception('An error partially decrypted ciphertext')
            else:
                return self.final_raw_decrypt(partial_decrypted_number1.ciphertext, partial_decrypted_number2.ciphertext)
                """encoding = self.final_raw_decrypt(partial_decrypted_number1.ciphertext, partial_decrypted_number2.ciphertext)
                if partial_decrypted_number1.exponent == 0:
                    return encoding
                else:
                    return EncodedNumber(encoding, partial_decrypted_number1.exponent).decode()"""
                #EncodedNumber.decode(encoding, max_exponent=partial_decrypted_number1.exponent)
        else:
            raise TypeError('Expected int type plaintext but got: %s' %
                            type(partial_decrypted_number1), type(partial_decrypted_number2))
...
class PaillierPublicKey(object):
    ...
    def raw_encrypt(self, plaintext, r_value=None):
        ...
        if plaintext > self.n:
            raise TypeError('plaintext should be less than n')
        else:
            # we chose g = n + 1, so that we can exploit the fact that
            # (n+1)^plaintext = n*plaintext + 1 mod n^2
            plaintext = plaintext % self.n if plaintext < 0 else plaintext
    ...
    def encrypt(self, value, precision=None, r_value=None):
        ...
        if isinstance(value, EncodedNumber):
            ...
            encoding = EncodedNumber.encode(value, precision) if isinstance(value, float) else EncodedNumber(value, 0)
...
class PaillierPrivateKey(object):
    ...
    def decrypt_encoded(self, encrypted_number, Encoding=None):
        ...
        if encoded > self.public_key.max_int:
            encoded = encoded - self.public_key.n
        return Encoding(encoded, encrypted_number.exponent)
...
class PartialDecryptedNumber(object):
...
class EncryptedNumber(object):
    ...
    def __add__(self, other):
        """Add an int, float, or `EncryptedNumber`"""
    ...
    def __mul__(self, other):
        ...
        if isinstance(other, EncryptedNumber):
            ...
            if isinstance(other, float):
                encoding = EncodedNumber.encode(other)
    ...
    def increase_exponent_to(self, new_exp):
        ...
        if new_exp < self.exponent:
            raise ValueError('New exponent %i should be larger than '
                             'old exponent %i' % (new_exp, self.exponent))
        return self * ((new_exp - self.exponent) // EncodedNumber.DEFAULT_EXPONENT)
    ...
    def _add_scalar(self, scalar):
        ...
        """
        #encoded = EncodedNumber.encode(scalar, max_exponent=self.exponent)
        #return self._add_encoded(encoded)
        return self._add_encrypted(self.public_key.encrypt(scalar))
    ...
    def _add_encrypted(self, other):
        ...
        if a.exponent > b.exponent:
            b = b.increase_exponent_to(a.exponent)
        elif a.exponent < b.exponent:
            a = a.increase_exponent_to(b.exponent)

        sum_ciphertext = a._raw_add(a.ciphertext(False), b.ciphertext(False))
        return EncryptedNumber(a.public_key, sum_ciphertext, max(a.exponent, b.exponent))
    ...
    def _raw_mul(self, plaintext):
        ...
        if plaintext >= self.public_key.n:
